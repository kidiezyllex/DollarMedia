  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-2 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-sm",
          className
        )}
      >
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-2 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-gray-500"
              )}
            >
...
export {
  ChartContainer as Chart, ChartContainer, ChartLegend,
  ChartLegendContent,
