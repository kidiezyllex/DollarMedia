    ...
    <div className="block lg:hidden bg-mainBackgroundV1 py-12 px-4 border-t-2 border-accent/30">
      <div className="max-w-md mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-4">
            <label className="text-base font-semibold text-secondary ml-1">Gói bạn quan tâm</label>
            <div className="grid gap-3">
              {Object.entries(packageNames).map(([id, name]) => (
                <label key={id} className="flex items-start space-x-3 cursor-pointer group">
                  <div className="relative flex items-center justify-center mt-1">
                    <input
                      type="checkbox"
                      className="peer appearance-none w-6 h-6 border-2 border-accent/30 rounded-sm checked:bg-secondary checked:border-secondary transition-all bg-white/5 cursor-pointer"
                      onChange={() => {
                        const numId = Number(id);
                        setSelectedPackages((prev) =>
                          prev.includes(numId) ? prev.filter((p) => p !== numId) : [...prev, numId]
                      ...
                      checked={selectedPackages.includes(Number(id))}
                    />
                    <div className="absolute opacity-0 peer-checked:opacity-100 text-amber-950 pointer-events-none transition-opacity">
          ...
          <div className="pt-4">
            <GoldButton
              type="submit"
              disabled={isSubmitting}
              className="w-full h-14 text-base group disabled:opacity-50"
            >
              {isSubmitting ? "ĐANG GỬI..." : "GỬI YÊU CẦU NGAY"}
              {!isSubmitting && <Icon path={mdiSend} size={0.8} />}
            </GoldButton>
          </div>
